...
describe("viewing a specific blog", () => {
  test("succeeds with a valid id", async () => {
    ...
    const specificBlog = await api.get(`/api/blogs/${response.body[0].id}`);

    const afterDeleteResponse = await api.get("/api/blogs");

    expect(specificBlog.body.title).toContain("Title test 1");
  });

  test("fails with statuscode 404 if blog does not exist", async () => {
    const blog = new Blog({
      title: "Blog to get id",
      author: "Blog to get id",
      url: "Blog to get id",
    });
    await blog.save();
    await blog.remove();
